...
export class EquiposComponent implements OnInit {
  ...
  private agregarJugadoresSecuencialmente(index: number) {
    ...
    this.equiposService.agregarJugador(
      ...
      error: (err) => {
        console.error('Error agregando jugador:', err);
        this.mensajeError = `Error agregando jugador "${jugador.nombre} ${jugador.apellido}": ${err.error?.message || 'Error desconocido'
          }`;
        this.guardando = false;
      }
    });
  ...
  eliminarEquipo(equipo: Equipo, event: Event) {
    event.stopPropagation();
    if (!this.userToken) return;
    const confirmacion = confirm(
      `¿Estás seguro de eliminar el equipo: ${equipo.nombre}?\nEsta acción eliminará también a todos sus jugadores.`
    );
    if (confirmacion) {
      this.equiposService.eliminarEquipo(equipo.id, this.userToken).subscribe({
        next: (resp) => {
          this.equiposView = this.equiposView.filter(e => e.id !== equipo.id);
          this.aplicarFiltro(); // Actualizamos la vista filtrada
          const mensaje = resp?.message || 'Equipo eliminado correctamente';
          this.mostrarMensajeExito(mensaje);
        },
        error: (err) => {
          console.error(err);
          this.mostrarMensajeError('No se pudo eliminar el equipo.');
        }
      });
    }
  }
